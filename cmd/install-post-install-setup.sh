...
install -Dm644 "$SCRIPTDIR/march.sh" /mnt/usr/local/sbin/march.sh
...
# Shared helpers for post-install scripts
if [[ -d "$SCRIPTDIR/func" ]]; then
    for func_file in "$SCRIPTDIR"/func/*.sh; do
        [[ -f "$func_file" ]] || continue
        install -Dm644 "$func_file" "/mnt/usr/local/sbin/func/$(basename "$func_file")"
    done
fi

if [[ -d "$SCRIPTDIR/env" ]]; then
    for env_file in "$SCRIPTDIR"/env/*.sh; do
        [[ -f "$env_file" ]] || continue
        install -Dm644 "$env_file" "/mnt/usr/local/sbin/env/$(basename "$env_file")"
    done
fi

...
ITEMS=(
    ...
    "IROOT_PARTITION_FSTYPE|string|Root filesystem type (ext4 is the only supported FS for now)"
    "IHOME_PARTITION_FSTYPE|string|Home filesystem type (ext4 is the only supported FS for now)"
    "ISWAP_TYPE|string|Swap type (zram, swapfile, or zram+swapfile)"
    ...
    "IKERNEL_CMDLINE|string|Kernel command line"

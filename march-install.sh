...
if [ ! -d "passwords" ] || [ -z "$(ls -A passwords)" ]; then
    ...
    echo "Please run march-users-gen.sh to generate user passwords."
    exit 1
fi

# Setup users from passwords/ directory
./march-setup-users.sh
